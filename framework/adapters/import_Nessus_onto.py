...
def hosts(dataFile):
    ...
    for i in report.findall("ReportHost"):
        ...
        if(standardName not in collected):
            ...
            osInfo = paperRUN.getStandardOpsysName(standardName, platform, duplicatesL, versionL, paperRUN.coll, paperRUN.collAssoc, paperRUN.con)
            osName = osInfo[0]
            osVersion = osInfo[1]
...
def store(sourceName,scope, data, hash, date, client):
        ...
        for key,val in data.items():
            ...
            for d in opsys.values():
                ...
                osVer = d.get('osVersion')
                standardName=' '.join((osName,osVer))
                #print(standardName)
