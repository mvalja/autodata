	...
	for i in root.findall("xmlns:Obj",ns):
		...
		for s in props:
			if(s.attrib.get('N') == "Name" and (s.tag!="xmlns:Nil",ns)):
				...
				pidentity = s.text
		if(pidentity!=None):
				output[pidentity]={"name":pname, "version":pversion,"vendor":pvendor, "installLocation": pinstallloc,
								   "lastChange": pinstalldate, "packageCode": ppackagecode, "regOwner": powner}
