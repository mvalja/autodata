...
def readFile(hostFile):
	...
	for sec in packet:
		...
		if (tools.isNotEqual('ARP',secelem[4].text) and tools.isNotEqual('DHCPv6',secelem[4].text) and tools.isNotEqual('ICMPv6',secelem[4].text) and  tools.isNotEqual('LLMNR',secelem[4].text) and ('.255' not in secelem[2].text) and ('.255' not in secelem[3].text) ):
			output.add((secelem[2].text,secelem[3].text,secelem[4].text ))
	return output

pprint(readFile("..\\data\\wireshark\\summary-packets_fixed.xml"))
...
def store(sourceName,scope, sourceIP, nodeName, data, hash, date, client):
	...
	for da in data:
		...
		#Create zones
		szone=tools.returnNetworkZone2args(source)
		dzone=tools.returnNetworkZone2args(destin)
