...
def store(sourceName,scope, data, hash, date, client):
        ...
        for key,val in data.items():
            ...
            for d in opsys.values():
                ...
                spKey = storeElementArango('element', 'softwareInstance', 'operatingSystem', standardName, osCertainty, client)
                storeAssocArango('elementAssoc', nodeKey, spKey, 'operatingsystemSoftwareproduct', client)

            for d in vuln:
                cve=d
                cveKey = storeElementArango('element', 'property', 'cve', cve, None, client)
