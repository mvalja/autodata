...
#dataD=paperRUN.readMyCSV('F:\ongoingWork\FOI\pcap\snart-fw.cleanfurnace1.se\psml\pcap_00.csv', ',', False)

def parseReadData(inputD):
	wireSet=set()
	for i in inputD:
		if((len(i[0])+len(i[4])+len(i[5]))>len(i[0]) and ',' not in i[0]):
			src=i[0]
			dst=i[3]
			destPortTCP=i[4]
			destPortUDP =i[5]
			...
			if(destPortTCP!=''):
				...
				finPort=destPortUDP
			if(int(finPort)<49152):
				wireSet.add((src,dst,finPort))
	return wireSet

#parsed=parseReadData(dataD)
...
#2#run('Wireshark', 'subnet','199.206.2.31','Winscanner','F:\ongoingWork\FOI\pcap\snart-fw.cleanfurnace1.se\psml\pcap_02.csv', tools.creationDate("F:\ongoingWork\FOI\pcap\snart-fw.cleanfurnace1.se\psml\pcap_02.csv"), createConArango('OntoLab'))